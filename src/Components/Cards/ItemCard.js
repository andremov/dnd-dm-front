...
import { deleteItem, modifyItem } from "../../Services/api";
...
export function ItemCard( { data } ) {
    ...
    function sendDelete() {
        setDeleting(true)
        deleteItem(data._id).then(r => {
            setDeleting(false)
        })
    }
    
    return <div
        className={'inventory-item-card'+(deleting? ' erasing':'')}
...
function ModifyItem( { data, callback } ) {
    ...
    function sendCreateItem() {
        ...
        setProgress(true)
        modifyItem(data._id, idata).then(r => {
            callback()
        })
    }
